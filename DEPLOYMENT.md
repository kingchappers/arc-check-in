...
- **Name:** `arc-check-in-app` (from `var.app_name`)
...
#### API Lambda
- **Name:** `arc-check-in-app-api` (from `${var.app_name}-api`)
...
#### Lambda Execution Role (`arc-check-in-app-lambda-role`)
...
```json
{
  ...
  "Resource": "arn:aws:lambda:...:function:arc-check-in-app*"
}
```
...
- **Log Group:** `/aws/apigateway/arc-check-in-app`
- **Retention:** 7 days
...
```bash
aws logs tail /aws/apigateway/arc-check-in-app --follow
```

View Lambda logs:
```bash
# Static file server
aws logs tail /aws/lambda/arc-check-in-app --follow

# API Lambda
aws logs tail /aws/lambda/arc-check-in-app-api --follow
```
